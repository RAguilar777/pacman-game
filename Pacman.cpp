...
void Pacman::draw(bool i_victory, sf::RenderWindow& i_window)
{
	...
	{
		if (animation_timer < DEATH_FRAMES * PACMAN_ANIMATION_SPEED)
		{
			animation_timer++;
	...
	else
	{
		...
		animation_timer = (1 + animation_timer) % (PACMAN_ANIMATION * PACMAN_ANIMATION_SPEED);
	}
}
...
void Pacman::update(unsigned char i_level, std::array<std::array<Cell, HEIGHT>, WIDTH>& i_map)
{
	...
	{
		// You can't turn in this direction if there's a wall there.
		if (0 == walls[0]) 
		{
			direction = 0;
		}
	...
	{
		position.x = CELL_SIZE * WIDTH - PACMAN_SPEED;
	}
	else if (CELL_SIZE * WIDTH <= position.x)
	{
