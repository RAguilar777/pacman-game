...
void draw_text(bool i_center, unsigned short i_x, unsigned short i_y, const std::string& i_text, sf::RenderWindow& i_window)
{
	...
	if (1 == i_center)
	{
		character_x = static_cast<short>(round(0.5f * (CELL_SIZE * WIDTH - character_width * i_text.substr(0, i_text.find_first_of('\n')).size())));
		character_y = static_cast<short>(round(0.5f * (CELL_SIZE * HEIGHT - FONT * (1 + std::count(i_text.begin(), i_text.end(), '\n')))));
	}
	...
	for (std::string::const_iterator a = i_text.begin(); a != i_text.end(); a++)
	{
		if ('\n' == *a)
		{
			if (1 == i_center)
			{
				...
				character_x = static_cast<short>(round(0.5f * (CELL_SIZE * WIDTH - character_width * i_text.substr(1 + a - i_text.begin(), i_text.find_first_of('\n', 1 + a - i_text.begin()) - (1 + a - i_text.begin())).size())));
			}
			else
			{
				character_x = i_x;
		...
		character_sprite.setTextureRect(sf::IntRect(character_width * (*a - 32), 0, character_width, FONT));
